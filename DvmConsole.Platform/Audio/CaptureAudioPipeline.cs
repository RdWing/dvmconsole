...
namespace DvmConsole.Platform.Audio
{
    ...
    public sealed class CaptureAudioPipeline : IAsyncDisposable
    {
            ...
            lock (_stateGate)
            {
                ...
                try
                {
                    var endTask = input.StartAsync(
                        ...
                        cancellationToken);
                    _input = input;
                    return AwaitEndAsync(endTask, completion);
                }
                catch
                {
                    ...
                    // failed input, clear the field and rethrow so a later
        ...
        /// <summary>
        /// Stops the capture stream. Idempotent.
        /// </summary>
        public Task StopAsync()
        {
            return Volatile.Read(ref _input)?.StopAsync() ?? Task.CompletedTask;
        }

        /// <summary>
        /// Stops and disposes the capture stream and disposes the injected
        /// factory. Idempotent.
        /// </summary>
