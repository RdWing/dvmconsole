...
public sealed class ReceiveSessionControllerTests
{
    ...
    private sealed record TestFrame(RadioMediaProtocol Protocol, uint PeerId, uint SourceId,
        uint DestinationId, byte? Slot, string CallType, string FrameType, string Subtype,
        ushort PacketSequence, uint StreamId, byte[] Payload) : IRadioMediaFrame;
