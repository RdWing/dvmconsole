...
internal sealed class ReceiveAudioRouteRegistry
{
    ...
    private readonly ConcurrentDictionary<ChannelId, string> sessionRoutes = [];
    private readonly Dictionary<string, ChannelId[]> sessionRouteSnapshots =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<ChannelId, bool> sessionFollowsSystemDefault = [];
    ...
    public bool TryAddSessionRoute(ChannelId channel, string deviceId)
    {
        lock (sessionRouteSync)
        {
            ...
            sessionRouteSnapshots.TryGetValue(deviceId, out ChannelId[]? current);
            var updated = new ChannelId[(current?.Length ?? 0) + 1];
    ...
    public bool TryRemoveSessionRoute(ChannelId channel, out string? deviceId)
    {
        lock (sessionRouteSync)
        {
            ...
            ChannelId[] current = sessionRouteSnapshots[deviceId];
            if (current.Length == 1)
            {
            ...
            var updated = new ChannelId[current.Length - 1];
            int index = 0;
            foreach (ChannelId candidate in current)
            {
    ...
    public void AddSessionPolicy(ChannelId channel, bool followsSystemDefault)
        => sessionFollowsSystemDefault.Add(channel, followsSystemDefault);

    public void RemoveSessionPolicy(ChannelId channel)
    ...
    public ChannelId[] GetSessionsForRoute(string deviceId)
    {
        lock (sessionRouteSync)
        {
            return sessionRouteSnapshots.TryGetValue(deviceId, out ChannelId[]? snapshot)
                ? snapshot.ToArray()
                : [];
        }
    }
    ...
    public ChannelId[] SelectSystemDefaultSessions(
        Func<ChannelId, bool> isActive)
    ...
    // Called while the coordinator gate is held.
    public ChannelId[] ExpandSharedRouteSessions(ChannelId[] requested)
    {
        if (requested.Length == 1)
        {
            lock (sessionRouteSync)
            {
                if (sessionRoutes.TryGetValue(requested[0], out string? requestedRouteId) &&
                    sessionRouteSnapshots.TryGetValue(requestedRouteId, out ChannelId[]? snapshot))
                {
        ...
        foreach (ChannelId channel in requested)
        {
