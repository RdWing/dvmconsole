...
internal static class RadioCallEpisodePolicy
{
    ...
    public static TimeSpan GetContinuationWindow(RadioMediaProtocol protocol)
        => protocol switch
        {
            RadioMediaProtocol.P25 => P25ContinuationWindow,
            RadioMediaProtocol.Dmr => TimeSpan.FromMilliseconds(
                RxJitterBufferSetting.MaximumDmrMilliseconds) + DmrPacketDuration,
            RadioMediaProtocol.Nxdn => TimeSpan.FromMilliseconds(
...
internal sealed class ReceiveCallEpisodeTracker
{
        ...
        DateTimeOffset observedAt)
    {
        ...
        lock (sync)
        {
            ...
            bool definitiveRestart = RadioReceiveTrafficClassifier.IsDefinitiveStart(traffic);
            ...
            {
                TimeSpan continuationWindow = RadioCallEpisodePolicy.GetContinuationWindow(
                    traffic.Protocol);
            ...
            TimeSpan window = RadioCallEpisodePolicy.GetContinuationWindow(traffic.Protocol);
        ...
        Func<ReceiveCallEpisodeSnapshot, bool>? canComplete = null)
    {
        lock (sync)
        {
            ...
            foreach (EpisodeState episode in episodes.Values
                         ...
                         .ToArray())
            {
                TimeSpan window = RadioCallEpisodePolicy.GetContinuationWindow(
    ...
    private static bool CanStartOrContinueEpisode(IRadioMediaFrame traffic)
    ...
    private readonly record struct ReceiveCallIdentity(
        ...
        string CallType)
    {
        public static ReceiveCallIdentity Create(string systemName, IRadioMediaFrame traffic)
            => new(
                systemName,
                traffic.Protocol,
                traffic.SourceId,
        ...
        EncryptionSnapshot encryption)
    {
        ...
        public void ObserveEncryption(IRadioMediaFrame traffic)
        {
