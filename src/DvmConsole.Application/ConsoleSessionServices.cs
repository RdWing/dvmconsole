...
/// </summary>
public sealed class ConsoleSessionServices : IAsyncDisposable
{
    ...
    private async Task DisposeCoreAsync()
    {
        ...
        long disposalStarted = timeProvider.GetTimestamp();
        for (int index = owned.Length - 1; index >= 0; index--)
        {
            ...
            TimeSpan remaining = overallDisposalTimeout - timeProvider.GetElapsedTime(disposalStarted);
...
public sealed class ConsoleSessionServiceScope
{
...
public readonly record struct ConsoleSessionServiceOwnership(
    string Scope,
    string Name);

public readonly record struct ConsoleSessionServiceDisposalTiming(
    string Scope,
    string Name,
    TimeSpan Duration);
