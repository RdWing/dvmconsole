...
internal sealed class ReceivePacketJitterBuffer<T>
{
    ...
    private long Add(long timestamp, TimeSpan duration)
        => timestamp + (long)Math.Round(duration.TotalSeconds * timeSource.TimestampFrequency);

    private static TimeSpan Multiply(TimeSpan duration, long multiplier)
        => TimeSpan.FromTicks(checked(duration.Ticks * multiplier));

    private TimeSpan Remaining(long now, long deadline)
        => deadline <= now
            ? TimeSpan.Zero
            : timeSource.GetElapsedTime(now, deadline);
