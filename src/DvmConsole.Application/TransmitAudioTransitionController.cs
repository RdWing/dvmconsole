...
internal sealed class TransmitAudioTransitionController
{
    ...
    private ChannelId[] suspendedChannels = [];
    ...
    public async Task PlayTalkPermitToneAsync(
        ...
        Func<TimeSpan>? getPttElapsed = null,
        ...
        TimeSpan? microphoneReadyAt = null)
    {
        try
        {
            ...
            string drainText = result.QueuedSamples is int queued &&
                ...
                ? $" queued {FormatAudioLevelDuration(queued)} / " +
                  $"consumed {FormatAudioLevelDuration(consumed)}"
                : string.Empty;
            ...
            string presentationLatencyText = result.MeasuredOutputPresentationLatency is TimeSpan outputLatency
                ...
                : result.PostDrainWaitDuration > TimeSpan.Zero
                    ...
                    : " No post-drain wait.";
            string pttText = getPttElapsed is null
                ? string.Empty
                : $" PTT sessions {transmitSessionsReadyAt?.TotalMilliseconds:0} ms, " +
                  ...
                  $"permit complete {getPttElapsed().TotalMilliseconds:0} ms (includes tone and post-drain wait).";
    ...
    internal static string FormatAudioLevelDuration(long sampleCount)
    {
        ...
        return $"{sampleCount / (double)PcmAudioFormat.Voice8KhzMono16Bit.SampleRate:0.0##} s";
    }
    ...
    {
        ChannelId[] channels = suspendedChannels;
        bool keptActive = suspendedChannelsKeptActive;
        var remaining = new List<ChannelId>(channels);
        var failures = new List<Exception>();
        foreach (ChannelId channel in channels)
        {
            if (!presentation.Capture(channel).AudioSuspended)
            {
            ...
            try
            {
                if (keptActive && routes.IsActive(channel))
                {
                    await routes
                        .SetGainAsync(channel, presentation.GetVolume(channel))
                        .ConfigureAwait(false);
                    bool enableLivePlayback = mute.ShouldEnableLivePlayback(
                        channel,
                        isTemporarilySuspended: false);
                    await routes.SetLivePlaybackEnabledAsync(channel, enableLivePlayback)
                        .ConfigureAwait(false);
                    await presentation.RunAsync(() => presentation.SetAudioSuspended(channel, false))
                        .ConfigureAwait(false);
                }
                else
                {
                    ...
                    if (!presentation.Capture(channel).AudioEnabled)
                    {
                        await presentation.RunAsync(() => presentation.SetAudioSuspended(channel, false)).ConfigureAwait(false);
                        remaining.Remove(channel);
                        continue;
                    }
                    await routes.StartAsync(channel).ConfigureAwait(false);
                    if (!routes.IsActive(channel))
                    {
                        throw new InvalidOperationException(
                            $"Receive audio for {presentation.Capture(channel).Name} could not be restored.");
                    }
                    await presentation.RunAsync(() => presentation.SetAudioSuspended(channel, false))
                        .ConfigureAwait(false);
                }
    ...
    public async Task MuteReceiveAudioAsync(string statusText)
    {
        ...
        {
            ChannelId[] receivingChannels = routes.LivePlaybackChannels.ToArray();
            if (receivingChannels.Length == 0)
                return;

            var suspended = new List<ChannelId>(receivingChannels.Length);
            var failures = new List<Exception>();
            foreach (ChannelId receivingChannel in receivingChannels)
            {
                try
                {
                    await routes
                    ...
                    await presentation.RunAsync(() => presentation.SetAudioSuspended(receivingChannel, true))
                        .ConfigureAwait(false);
                }
                catch (Exception exception)
                {
                    failures.Add(new InvalidOperationException(
                        $"Receive output for {presentation.Capture(receivingChannel).Name} could not be muted.",
                        exception));
                }
            }
