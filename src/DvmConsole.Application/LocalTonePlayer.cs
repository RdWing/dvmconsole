...
internal sealed class AudioOutputRouteResolver : IAudioOutputRouteResolver
{
    ...
    {
        this.delayAsync = delayAsync ?? SystemApplicationDelay.Instance.DelayAsync;
    }
...
internal sealed class LocalTonePlayer : IAsyncDisposable
{
        ...
        IMonotonicTimeSource? monotonicTimeSource = null)
    {
        ...
        this.delayAsync = delayAsync ?? SystemApplicationDelay.Instance.DelayAsync;
        this.monotonicTimeSource = monotonicTimeSource ?? SystemReceiveWorkQueueScheduler.Instance;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var timing = new MonotonicElapsedTimer(monotonicTimeSource);
    ...
    private sealed class MonotonicElapsedTimer(IMonotonicTimeSource timeSource)
    {
        private readonly long startedAt = timeSource.GetTimestamp();

        public TimeSpan Elapsed => timeSource.GetElapsedTime(
            startedAt,
