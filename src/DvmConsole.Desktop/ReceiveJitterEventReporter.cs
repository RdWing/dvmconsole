...
internal sealed class ReceiveJitterEventReporter
{
        ...
        DateTimeOffset now)
    {
        ...
        lock (sync)
        {
            var key = new StreamKey(channel, timing.Traffic.StreamId);
        ...
        uint streamId)
    {
        ...
        lock (sync)
        {
            var key = new StreamKey(channel, streamId);
    ...
    public void Reset(ChannelViewModel channel)
    {
        ...
        lock (sync)
        {
            ...
            while (node is not null)
            {
                ...
                if (ReferenceEquals(node.Value.Channel, channel))
                {
    ...
    private readonly record struct StreamKey(ChannelViewModel Channel, uint StreamId);
