...
/// <summary>
/// Filesystem TAR persistence behind the portable recording-store contract.
...
/// </summary>
public sealed class OpusRecordingStore : IRecordingStore, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        lock (sync)
        {
            ...
            lock (sync)
            {
                ...
                {
                    stream.Dispose();
                    throw new ObjectDisposedException(nameof(OpusRecordingStore));
                }
    ...
    public ValueTask<Stream> OpenReadAsync(
        RecordingId id,
        CancellationToken cancellationToken = default)
        => ValueTask.FromResult(OpenPlaybackSource(id, cancellationToken).Stream);

    public ValueTask<RecordingPlaybackSource> OpenPlaybackAsync(
        RecordingId id, CancellationToken cancellationToken = default)
        => ValueTask.FromResult(OpenPlaybackSource(id, cancellationToken));

    private RecordingPlaybackSource OpenPlaybackSource(RecordingId id, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        CallRecordingMetadata metadata = LoadRecordings(cancellationToken)
            .FirstOrDefault(item => RecordingIdentityEquals(item, id))
            ?? throw new KeyNotFoundException($"Recording '{id}' is not in the recording catalog.");
        ...
        Stream stream = new FileStream(
            ...
            64 * 1024,
            FileOptions.Asynchronous);
        return new(stream, metadata.ToCallIdentity());
    }
    ...
    public IReadOnlyList<CallRecordingMetadata> LoadRecordings(
        CancellationToken cancellationToken = default)
        => ScanCatalog(RootPath, 0, DateTimeOffset.UtcNow, cancellationToken).Recordings;

    private DvmConsole.Operations.CatalogScanHealth? catalogHealth;
    internal DvmConsole.Operations.CatalogScanHealth? CatalogHealth => Volatile.Read(ref catalogHealth);

    private RecordingCatalogScanResult ScanCatalog(string path, int retention, DateTimeOffset now,
        CancellationToken cancellationToken)
    {
        var scan = catalogStore.Scan(path, retention, now, cancellationToken);
        Volatile.Write(ref catalogHealth, new(scan.ScannedFiles, scan.Recordings.Count,
            scan.PrunedFiles, scan.DamagedFiles, scan.InaccessiblePaths, scan.Duration));
        return scan;
    }
    ...
    public Task<RecordingCatalogScanResult> LoadAndPruneRecordingsAsync(
        ...
        => Task.Run(() =>
        {
            lock (sync)
            {
                var result = ScanCatalog(rootPath, pruneExpired && CanWriteRecordings ? retentionDays : 0,
                    now ?? DateTimeOffset.UtcNow, cancellationToken);
                if (result.PrunedFiles > 0) Interlocked.Increment(ref catalogRevision);
                return result;
            }
        }, cancellationToken);
    ...
    public int PruneExpired(DateTimeOffset? now = null)
    {
        lock (sync)
        {
            int pruned = retentionDays <= 0 || !CanWriteRecordings ? 0 :
                ScanCatalog(rootPath, retentionDays, now ?? DateTimeOffset.UtcNow, CancellationToken.None).PrunedFiles;
            if (pruned > 0) Interlocked.Increment(ref catalogRevision);
            return pruned;
        }
    }
    ...
    public bool DeleteRecording(CallRecordingMetadata metadata)
    {
        lock (sync)
        {
            bool deleted = CanWriteRecordings && catalogStore.Delete(rootPath, metadata);
            if (deleted) Interlocked.Increment(ref catalogRevision);
            return deleted;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await RecordingFileCodec.EncodeWaveRangeAsync(
                snapshot.WavePath,
                temporaryOpusPath,
    ...
    {
        private readonly OpusRecordingStore owner;
        ...
        public ValueTask CheckpointAsync(CancellationToken cancellationToken = default)
        {
            ...
            // The capture service serializes this with writes and finalization.
