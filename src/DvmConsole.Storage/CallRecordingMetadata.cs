...
{
    internal RecordingCallIdentity ToCallIdentity()
        => new(UtcStartTime, SystemName, ChannelName, Direction, Protocol, SubscriberId,
            TalkgroupId, StreamId, ReceiveEpisodeId);
    ...
    internal void NormalizeCompatibilityFields()
    {
        if (SchemaVersion < CurrentSchemaVersion)
            return;
        RecordingEncryptionSchema.ApplyToMetadata(
            this,
            RecordingEncryptionSchema.FromMetadata(this),
            EncryptionProtocolLabels.ParseProtocol(Protocol));
    }
    ...
    public CallRecordingEncryptionState EffectiveEncryptionState =>
        RecordingEncryptionSchema.FromMetadata(this).State;
    ...
    public string EffectiveEncryptionAlgorithm => !string.IsNullOrWhiteSpace(EncryptionAlgorithm)
        ...
        : EncryptionProtocolLabels.AlgorithmDisplayName(
            EncryptionProtocolLabels.ParseProtocol(Protocol),
            RecordingEncryptionSchema.FromMetadata(this).AlgorithmId);
