...
/// <summary>
/// Queues recording operations against Application's RX/TX recording lifecycle
/// and the shared TAR store. Hosts provide immutable channel descriptors.
/// </summary>
public sealed class CallRecordingManager :
    IDisposable,
    IAsyncDisposable,
    IRecordingFinalizationHealthSource,
    IRecordingCatalogHealthSource,
    IReceiveRecordingSession,
    IRecordingRetentionControl,
    ITransmitRecordingSink,
    IRecordingCallAttachmentSource
{
    public const int DefaultRetentionDays = 7;
    private const int RecordingWorkCapacity = 2_048;

    private readonly OpusRecordingStore store;
    private readonly bool ownsStore;
    private readonly HashSet<Guid> capturedRecordings = [];
    ...
    internal CallRecordingManager(
        ...
        : this(reportFault => new OpusRecordingStore(rootPath, reportFault, retentionDays,
            finalizationQueueCapacity, finalizeRecording), ownsStore: true, faultHandler, retentionDays,
            shouldRecordSource, resolveSubscriberAlias, recordingWorkCapacity, samplePool)
    {
    }

    /// <summary>Creates a session capture queue over a host-owned durable store.</summary>
    public CallRecordingManager(OpusRecordingStore store,
        Action<ChannelId, Exception>? faultHandler = null,
        int retentionDays = DefaultRetentionDays,
        Func<ChannelId, uint, bool>? shouldRecordSource = null,
        Func<ChannelId, uint, string>? resolveSubscriberAlias = null)
        : this(_ => store ?? throw new ArgumentNullException(nameof(store)), ownsStore: false,
            faultHandler, retentionDays, shouldRecordSource, resolveSubscriberAlias, RecordingWorkCapacity, null)
    {
    }

    private CallRecordingManager(Func<Action<ChannelId, Exception>, OpusRecordingStore> createStore,
        bool ownsStore, Action<ChannelId, Exception>? faultHandler, int retentionDays,
        Func<ChannelId, uint, bool>? shouldRecordSource, Func<ChannelId, uint, string>? resolveSubscriberAlias,
        int recordingWorkCapacity, ArrayPool<short>? samplePool)
    {
        ...
        {
            if (ownsStore) store.DisposeAsync().AsTask().GetAwaiter().GetResult();
            throw;
        }
        service.StateChanged += HandleCaptureStateChanged;
    ...
    bool IReceiveRecordingSession.CanWrite => CanWriteRecordings;
    event Action<ChannelId>? IReceiveRecordingSession.StateChanged
    {
        add => RecordingStateChanged += value;
        remove => RecordingStateChanged -= value;
    }
    ReceiveRecordingState IReceiveRecordingStateSource.CaptureState(ChannelId channel)
    {
        lock (recordingStateSync)
        {
            var value = recordingStates.GetValueOrDefault(channel);
            return new(value.IsRecording, value.IsFinalizing, value.Fault);
        }
    }
    IReadOnlyDictionary<ChannelId, ReceiveRecordingState> IReceiveRecordingStateSource.CaptureStates(
        IReadOnlyList<ChannelId> channels)
    {
        lock (recordingStateSync)
        {
            var result = new Dictionary<ChannelId, ReceiveRecordingState>(channels.Count);
            foreach (ChannelId channel in channels)
            {
                var value = recordingStates.GetValueOrDefault(channel);
                result.Add(channel, new(value.IsRecording, value.IsFinalizing, value.Fault));
            }
            return result;
        }
    }
    Task IReceiveRecordingSession.DrainAsync(CancellationToken cancellationToken)
        => DrainAcceptedWorkAsync(cancellationToken);
    ...
    public async Task CheckpointAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!work.Writer.TryWrite(RecordingWorkItem.Operation(default,
                () => service.CheckpointAsync(cancellationToken), completion, propagateStorageFailure: true)))
            throw new InvalidOperationException("The recording queue is no longer accepting checkpoints.");
    ...
    public async Task<RecordingRetentionPreview> PreviewRetentionAsync(int days, CancellationToken cancellationToken = default)
    {
        new RecordingRetentionPolicy(days).Validate();
        DateTimeOffset now = DateTimeOffset.UtcNow;
        var scan = await PreviewRecordingPolicyAsync(RootPath, days, now, cancellationToken).ConfigureAwait(false);
        return new(days == 0 ? null : now.AddDays(-days), scan.RetentionCandidates);
    }

    public async Task<int> PruneExpiredAsync(CancellationToken cancellationToken = default)
    {
        await FlushAsync(cancellationToken).ConfigureAwait(false);
        var scan = await store.LoadAndPruneRecordingsAsync(true, cancellationToken: cancellationToken).ConfigureAwait(false);
        return scan.PrunedFiles;
    }
    ...
    DvmConsole.Operations.CatalogScanHealth? IRecordingCatalogHealthSource.CatalogHealth
        => store.CatalogHealth;
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        finally
        {
            service.StateChanged -= HandleCaptureStateChanged;
            store.RecordingFinalized -= HandleStoreRecordingFinalized;
            if (ownsStore) await store.DisposeAsync().ConfigureAwait(false);
        }
    }
    ...
    private async Task ProcessWorkAsync()
    {
        ...
        try
        {
            await foreach (RecordingWorkItem item in work.Reader.ReadAllAsync().ConfigureAwait(false))
            {
                ...
                if (failure is null || (!item.PropagateStorageFailure && IsRecordingStorageFailure(failure)))
    ...
    private void HandleCaptureStateChanged(object? sender, RecordingOperationState state)
    {
        if (ownsStore) return;
        lock (recordingStateSync) capturedRecordings.Add(state.RecordingId.Value);
    }
        ...
        RecordingFinalizationResult result)
    {
        ...
        if (!ownsStore)
        {
            lock (recordingStateSync)
            {
                if (result.Descriptor?.RecordingId is not Guid id || !capturedRecordings.Contains(id)) return;
                if (result.Error is null) capturedRecordings.Remove(id);
            }
            if (result.Error is { } failure) ReportFault(channelId, failure);
        }
        if (result.IsPlayable && result.Error is null)
            RecordingAttached?.Invoke(result.Metadata!.ToCallIdentity());
        ChannelRecordingState before;
        lock (recordingStateSync)
            before = recordingStates.GetValueOrDefault(channelId);
        PublishStateChangeIfNeeded(channelId, before);
    ...
    private readonly record struct RecordingWorkItem(
        ...
        bool PropagateStorageFailure = false)
    {
        ...
        public static RecordingWorkItem Operation(
            ...
            bool propagateStorageFailure = false)
            => new(RecordingWorkKind.Operation, channelId, null, 0, 0, 0, null, default, operation, completion, propagateStorageFailure);
