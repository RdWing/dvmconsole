...
public sealed partial class DocumentationCatalog : IConsoleHelpCatalog
{
    ...
    public static DocumentationCatalog OpenDefault()
        => Open(Path.Combine(BundledResources.Root(AppContext.BaseDirectory), "Documentation"));

    public static DocumentationCatalog Open(string root)
    {
        string manifestPath = Path.Combine(root, "manifest.json");
        DocumentationManifest manifest = JsonSerializer.Deserialize(
            File.ReadAllText(manifestPath),
            DocumentationJsonContext.Default.DocumentationManifest)
