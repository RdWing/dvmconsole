...
public static class OperationalHealthPresentation
{
    ...
    {
        if (health is null) return $"{label}: unavailable";
    ...
    public static string FormatCatalog(CatalogScanHealth? health)
        => health is null ? "TAR catalog: not scanned" : $"TAR catalog {health.Loaded:N0}/{health.FilesSeen:N0} loaded · " +
