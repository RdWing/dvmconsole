...
internal sealed class TonePresentationController
{
    ...
    public TonePatternDocument ExportPatterns() => new()
    {
        TonePresets = workspace.MutableTonePresets.Select(ToTonePresetSetting).ToList()
    };

    public void ImportPatterns(TonePatternDocument document)
    {
        document.Validate();
        var names = workspace.MutableTonePresets.Select(preset => preset.Name).ToHashSet(StringComparer.OrdinalIgnoreCase);
        var additions = new List<TonePresetViewModel>();
        foreach (var preset in document.TonePresets)
        {
            string original = preset.Name.Trim();
            string name = original;
            for (int suffix = 2; !names.Add(name); suffix++) name = $"{original[..Math.Min(original.Length, 150)]} ({suffix})";
            additions.Add(new TonePresetViewModel(new TonePresetSetting
            {
                Name = name,
        ...
        foreach (var preset in additions) workspace.MutableTonePresets.Add(preset);
        PersistTonePresets();
        session.BeginUndoableAction("Tone patterns imported", () =>
        {
            foreach (var preset in additions) workspace.MutableTonePresets.Remove(preset);
            PersistTonePresets();
            return ValueTask.CompletedTask;
        });
        session.SetTransmitStatus($"Imported {additions.Count} tone patterns. Existing names were retained; duplicates were renamed.");
    }
