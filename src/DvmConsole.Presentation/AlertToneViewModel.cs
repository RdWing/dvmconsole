...
{
    private readonly ILegacyAlertToneFiles files;

    public AlertToneViewModel(AlertToneSetting setting, ILegacyAlertToneFiles files)
    {
        ...
        FileName = string.IsNullOrWhiteSpace(setting.FileName)
            ? files.GetFileName(setting.FilePath)
            : files.GetFileName(setting.FileName);
    ...
    public string StorageText => Guid.TryParse(AssetId, out _)
        ...
        : FilePath;
    public bool IsAvailable => Guid.TryParse(AssetId, out _) || files.Exists(FilePath);
